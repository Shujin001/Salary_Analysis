...
with tab2:
    col1, col2, col3 = st.tabs(["Experience vs Salary", "Job Title Salary", "Other Factors"])

    with col1:
        st.subheader("Salary by Experience Level")
        fig1, ax1 = plt.subplots()
        exp_df = df[['experience_level', 'salary_in_usd']].dropna()
        sns.boxplot(x='experience_level', y='salary_in_usd', data=exp_df, ax=ax1)
        st.pyplot(fig1)
    ...
    with col3:
        st.subheader("Salary by Company Size")
        fig3, ax3 = plt.subplots()
        sns.barplot(x='company_size', y='salary_in_usd', data=df, ax=ax3)
        st.pyplot(fig3)

        st.subheader("Salary by Employment Type")
        fig4, ax4 = plt.subplots()
        sns.boxplot(x='employment_type', y='salary_in_usd', data=df, ax=ax4)
        st.pyplot(fig4)
